...
export const RanklabApiFp = function(configuration?: Configuration) {
    return {
        ...
        gamesList(options?: any): (fetch?: FetchAPI, basePath?: string) => Promise<Array<Game>> {
            const localVarFetchArgs = RanklabApiFetchParamCreator(configuration).gamesList(options);
            return (fetch: FetchAPI = isomorphicFetch, basePath: string = BASE_PATH) => {
                return fetch(basePath + localVarFetchArgs.url, localVarFetchArgs.options).then((response) => {
                    if (response.status >= 200 && response.status < 300) {
                        return response.json();
...
export const RanklabApiFactory = function (configuration?: Configuration, fetch?: FetchAPI, basePath?: string) {
    return {
         ...
         * @throws {RequiredError}
         */
        gamesList(options?: any) {
            return RanklabApiFp(configuration).gamesList(options)(fetch, basePath);
        },
        /**
         * 
         * @param {*} [options] Override http request option.
...
export class RanklabApi extends BaseAPI {
     ...
     * @memberof RanklabApi
     */
    public gamesList(options?: any) {
        return RanklabApiFp(this.configuration).gamesList(options)(this.fetch, this.basePath);
    }
